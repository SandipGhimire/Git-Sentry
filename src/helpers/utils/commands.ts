...
/**
 * Adds a command to a Git hook script inside the custom .gitsentry directory.
 *
...
const addHookCommand = (command: string, file: string): HookResult => {
  try {
    ...
    const gitSentryIgnore = path.join(hooksDir, ".gitignore");
    if (!fs.existsSync(gitSentryIgnore)) {
